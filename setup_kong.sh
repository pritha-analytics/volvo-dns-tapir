...
echo "==========================================="
echo "Configuring Cache Memory Layer..."
echo "==========================================="

echo "Adding Proxy Cache Plugin (Exact Match via Hash)..."
# We use x-prompt-hash header to allow proxy-cache to cache POST requests based on payload content
curl -i -X POST http://localhost:8001/services/openrouter-service/plugins \
  ...
  --data "config.storage_ttl=3600" \
  --data "config.vary_headers[]=x-prompt-hash" \
  --data "config.vary_headers[]=x-bypass-cache"
